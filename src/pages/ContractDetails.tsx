...
export default function ContractDetails() {
  ...
  const [pagamentoEfetuado, setPagamentoEfetuado] = useState<any>(null)
  ...
  useEffect(() => {
    async function load() {
      ...
      if (error || !data) {
        ...
        const { data: pagEfetuadoData } = await supabase
        ...
        if (pagEfetuadoData) {
          setPagamentoEfetuado(pagEfetuadoData)
        }
      }
      setLoading(false)
    }
    ...
    <div className="container max-w-4xl mx-auto py-8 px-4 animate-fade-in-up">
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4 flex flex-row items-center justify-between">
              ...
              <Button
                ...
                disabled={recalculando || pagamentoEfetuado}
              >
                ...
                <>
                  ...
                  {pagamentoEfetuado ? (
                    <div className="pt-4 border-t border-slate-100 animate-fade-in">
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex flex-col items-center justify-center text-center space-y-1">
                        <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mb-1">
                          <Check className="w-5 h-5 text-green-600" />
                        </div>
                        <span className="font-semibold text-green-800">Pagamento Processado</span>
                        <span className="text-xs text-green-600 font-mono">
                          Transação: {pagamentoEfetuado.transaction_id}
                        </span>
                        <span className="text-xs text-green-600">
                          Data:{' '}
                          {format(
                            parseISO(pagamentoEfetuado.data_pagamento),
                            "dd/MM/yyyy 'às' HH:mm",
                          )}
                        </span>
                      </div>
                    ...
                    pagamento &&
                    pagamento.valor_final > 0 &&
                    contrato.status !== 'cancelado' && (
                      <div className="pt-4 border-t border-slate-100 animate-fade-in">
                        <Button
                          className="w-full bg-green-600 hover:bg-green-700 text-white h-12 text-base font-medium shadow-sm transition-all"
      ...
      {showPaymentDialog && (
        <PaymentDialog
          ...
          contratoId={id!}
          freelancerId={contrato.freelancer_id}
          valorFinal={pagamento.valor_final}
          freelancerNome={freelancer?.nome_completo}
          onSuccess={(data) => setPagamentoEfetuado(data)}
        />
      )}
    </div>
  )
